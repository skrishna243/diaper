...
RSpec.describe "AuditsRequests", type: :request do
  ...
  describe "while signed in as an organization admin" do
    ...
    describe "GET #show" do
      it "is successful" do
        ...
        get audits_url(default_params.merge(id: audit.to_param))
        expect(response).to be_successful
      end
    end
    ...
    describe "GET #new" do
      it "is successful" do
        get new_audit_url(default_params)
        expect(response).to be_successful
      end
    end
    ...
    describe "GET #edit" do
      it "is successful if the status of audit is `in_progress`" do
        audit = create(:audit, organization: @organization)
        get edit_audit_url(default_params.merge(id: audit.to_param))
        expect(response).to be_successful
      end
    ...
    describe "POST #create" do
      context "with valid params" do
        it "creates a new Audit" do
          expect do
            post audits_url(default_params), params: { audit: valid_attributes }
          end.to change(Audit, :count).by(1)
        end

        it "creates a new Audit with status as `in_progress` if `save_progress` is passed as a param" do
          expect do
            post audits_url(default_params), params: { audit: valid_attributes, save_progress: '' }
        ...
        it "creates a new Audit with status as `confirmed` if `confirm_audit` is passed as a param" do
          expect do
            post audits_url(default_params), params: { audit: valid_attributes, confirm_audit: '' }
      ...
      context "with invalid params" do
        subject do
          post audits_url(default_params), params: { audit: invalid_attributes }
          response
        end

        it "will not save an invalid audit" do
          expect { subject }.to_not change(Audit, :count)
        end
    ...
    describe "DELETE #destroy" do
      setup do
        @audit = build(:audit, organization: @organization)
      end

      subject do
        delete audit_url(default_params.merge(id: @audit.to_param))
      end
