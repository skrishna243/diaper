RSpec.describe "Barcode management", type: :system do
  ...
  context "While viewing the barcode items index page" do
    ...
    it "only shows the barcodes created within the organization" do
  ...
  context "With organization-specific barcodes" do
    ...
    it "updates an existing barcode" do
      create(:item)
      barcode
    ...
    it "updates an existing barcode with empty attributes" do
      barcode
      visit url_prefix + "/barcode_items/#{barcode.id}/edit"
  ...
  it "can filter the #index by item type" do
    Item.delete_all
  ...
  it "can filter the #index by canonical item type" do
    Item.delete_all
  ...
  it "can filter the #index by barcode value" do
    Item.delete_all
    b = create(:barcode_item, organization: @organization)
    create(:barcode_item, organization: @organization)
    visit url_prefix + "/barcode_items"
  ...
  it "lists items in filter in alphabetical order" do
  ...
  it "adds a new barcode with empty attributes" do
    visit url_prefix + "/barcode_items/new"
    click_button "Save"
