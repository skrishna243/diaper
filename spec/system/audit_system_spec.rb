RSpec.describe "Audit management", type: :system do
  ...
  context "while signed in as a normal user" do
    ...
    it "does not permit user to visit the audits #index page" do
      visit url_prefix + "/audits"
      expect(page).to have_content("Access Denied")
    end

    it "does not permit user to visit audits #new page" do
      visit url_prefix + "/audits/new"
      expect(page).to have_content("Access Denied")
    end

    it "does not permit user to visit the audits #edit page" do
      visit url_prefix + "/audits/1/edit"
      expect(page).to have_content("Access Denied")
    end

    it "does not permit user to visit the audits #show page" do
  ...
  context "while signed in as an organization admin" do
    ...
    it "can save progress of an audit" do
    ...
    it "can resume the audit that is in progress" do
      item = create(:item)
    ...
    it "can delete the audit that is in progress", js: true do
      item = create(:item)
      storage_location = create(:storage_location, :with_items, item: item, item_quantity: 10)
      audit = create(:audit, :with_items, storage_location: storage_location, item: storage_location.items.first, item_quantity: quantity)
      visit url_prefix + "/audits/" + audit.to_param
    ...
    it "can confirm the audit from the #new page", js: true do
      storage_location = create(:storage_location, :with_items, organization: @organization)
      visit url_prefix + "/audits"
      click_link "New Audit"
      select storage_location.name, from: "From storage location"
    ...
    it "can confirm the audit from the #edit page", js: true do
      item = create(:item)
      storage_location = create(:storage_location, :with_items, item: item, item_quantity: 10)
      audit = create(:audit, :with_items, storage_location: storage_location, item: storage_location.items.first, item_quantity: quantity)
    ...
    it "can edit the audit that is confirmed" do
      item = create(:item)
      storage_location = create(:storage_location, :with_items, item: item, item_quantity: 10)
    ...
    it "can delete the audit that is confirmed", js: true do
      item = create(:item)
      storage_location = create(:storage_location, :with_items, item: item, item_quantity: 10)
      audit = create(:audit, :with_items, storage_location: storage_location, item: storage_location.items.first, item_quantity: quantity, status: :confirmed)
      visit url_prefix + "/audits/" + audit.to_param
    ...
    it "can finalize the audit", js: true do
      item = create(:item)
      storage_location = create(:storage_location, :with_items, item: item, item_quantity: 10)
      audit = create(:audit, :with_items, storage_location: storage_location, item: storage_location.items.first, item_quantity: quantity, status: :confirmed)
      visit url_prefix + "/audits/" + audit.to_param
    ...
    it "creates an adjustment with the differential when audit is finalized", js: true do
      item = create(:item)
      item_quantity = 10
    ...
    it "Finalized audit is immutable", js: true do
      item = create(:item)
      storage_location = create(:storage_location, :with_items, item: item, item_quantity: 10)
      audit = create(:audit, :with_items, storage_location: storage_location, item: storage_location.items.first, item_quantity: quantity, status: :confirmed)
      visit url_prefix + "/audits/" + audit.to_param
    ...
    it "can not delete the audit that is finalized", js: true do
      item = create(:item)
      storage_location = create(:storage_location, :with_items, item: item, item_quantity: 10)
      audit = create(:audit, :with_items, storage_location: storage_location, item: storage_location.items.first, item_quantity: quantity, status: :confirmed)
      visit url_prefix + "/audits/" + audit.to_param
